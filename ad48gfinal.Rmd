...
install.packages(c("httr", "zoo", "readr", "here", "renv", "data.table", "tidyverse", "ggplot2", "plotly", "dplyr", "stringr", "readxl","maps" ))
...
In this analysis, we aim to explore various socioeconomic indicators such as Human Development Index (HDI), life expectancy, education levels, political regimes, and economic measures like Gross National Income (GNI) per capita. The objective is to clean, merge, and analyze datasets from multiple sources, filling in missing values using regression-based imputation, and derive insights through visualizations and statistical models. (need to add a sentence on decision trees)
...
Using the HDI data, specifically the `hdiscore` column, we are creating four different categories of countries based on their HDI score levels. The countries in each category are selected randomly and will be used for further analysis and visualization. 
...
```{r}
ggplot(expected_years_of_schooling_total, aes(x = Year, y = Value, color = Value, group = `Country Name`)) +
  geom_line(alpha = 0.7) +
  scale_color_viridis_c() +
  theme_minimal()
```
...
necessary_columns <- c("Country Name", "Year", "LifeExpectancy", "GNI_Per_Capita", 
                       "MeanYearsOfSchooling", "Value", "HDI")

# Select only the gerekli columns from merged_data
model_data <- merged_data %>%
  select(all_of(necessary_columns))

model <- lm(HDI  ~ LifeExpectancy + GNI_Per_Capita + MeanYearsOfSchooling + Value, data=model_data )
summary(model)

# Specify the columns to check
columns_to_check <- c("LifeExpectancy", "GNI_Per_Capita", "MeanYearsOfSchooling", "Value", "HDI")
...
predict_or_calculate_hdi <- function(row, coefficients) {
  ...
  if (!is.na(row$Value)) {
    predicted_value <- predicted_value + coefficients["Value"] * row$Value
  }
...
model_data_with_predictions <- model_data %>%
  left_join(
    completed_rows %>%
      select(`Country Name`, Year, LifeExpectancy, GNI_Per_Capita, MeanYearsOfSchooling, Value, HDI),
  ...
  mutate(
    ...
    Value = ifelse(is.na(Value_original), Value_predicted, Value_original),
...
model2 <- lm(HDI  ~ LifeExpectancy + GNI_Per_Capita + MeanYearsOfSchooling + Value, data=model_data_with_predictions )
summary(model2)
```
...
filled_counts <- rows_with_one_missing %>%
  summarise(
    ...
    Value = sum(is.na(Value) & !is.na(completed_rows$Value)),
